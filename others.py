...
# Переменные / функции
bot_token = '1705238365:AAFsNBksiWZHtnAs6qfP-r5tceLZtgxl6e8' #
admin_user_id = 1778728096 #
channel_id = 1778728096 #

token = '' # qiwi token
phone = '+380685814496' # qiwi number

secret = '' # CRYSTAL PAY SECRET 1
login = '' # CRYSTAL PAY LOGIN

manager = "ooHhNoo" # username мэнеджера без @
group = "" # канал с отзывами, готовый вариант, можно оставить

bot_name = "Lightning77_bot" #username бота без @
...
with sqlite3.connect("evidence.db") as con:
cur = con.cursor()
cur.execute("INSERT INTO `users` (`user_id`, `user_name`, `balance`, `referal`,
 
`referal_count`) VALUES(?,?,?,?,?)", (user_id, user_name, 0, 0, 0))
except:
pass
